...
namespace System.Globalization
{
    ...
    public class VietnameseCalendar : Calendar
    {
        ...
        /// <summary>
        /// Can cua gio cua ngay voi JDN nay.
        /// </summary>
        public virtual string GetHourName(DateTime time)
        {
            int jdn = (int)Math.Floor((double)JulianDayNumber(time));
            //gio Ti
            int selestindexTi = (jdn - 1) * 2 % 10;
            ...
            int selestindex = (selestindexTi + t) % 10;

            String selestialStemsHour = selestialStems[selestindex];
            String terrestHour = terrestrialBranches[t % 12];

            return selestialStemsHour + " " + terrestHour;
        }

        public virtual string GetHourName(DateTime time, out string selestialStemsHour, out String terrestHour)
        {
            int jdn = (int)Math.Floor((double)JulianDayNumber(time));
            //gio Ti
            int selestindexTi = (jdn - 1) * 2 % 10;
            ...
            selestialStemsHour = selestialStems[selestindex];
            terrestHour = terrestrialBranches[t % 12];
        ...
        /// <param name="selestial"></param>
        /// <param name="teresstrial"></param>
        /// <returns></returns>
        public static string GetYearName(int year, out String selestial, out String teresstrial)
        {
            CheckYearRange(year);
            selestial = selestialStems[(year + 6) % 10];
            teresstrial = terrestrialBranches[(year + 8) % 12];
        ...
        public static string GetDayName(DateTime date, out String selestial, out String teresstrial)
        {
            CheckTicksRange(date.Ticks);
            int jd = (int)Math.Floor((double)JulianDayNumber(date));
            selestial = selestialStems[(jd + 9) % 10];
            teresstrial = terrestrialBranches[(jd + 1) % 12];
        ...
        /// <param name="teresstrial"></param>
        /// <returns></returns>
        public static string GetMonthName(int year, int month, out String selestial, out String teresstrial)
        {
            VietnameseCalendar calendar = new VietnameseCalendar();
            int leapMonth = calendar.GetLeapMonth(year, 0);
            CheckMonthRange(month, leapMonth);
            int i = ((leapMonth > 0 && month >= leapMonth) ? -1 : 0);
            selestial = selestialStems[(year * 12 + month + i + 3) % 10];
            teresstrial = terrestrialBranches[(month + i + 1) % 12];
        ...
        public static string GetPropitiousHour(DateTime dt, bool withHours)
        {
            withHours = true;
            int jdn = JulianDayNumber(dt);
